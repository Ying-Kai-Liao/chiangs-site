...
export default function ModernCarousel() {
  ...
  return (
    <div className="aspect-[4/5] h-[50vw] min-h-[200px] max-h-[400px] bg-black relative overflow-hidden m-5 rounded-lg">
      <button
        onClick={() => {
          setPrevIdx(idx);
          setIdx((pv) => pv - 1);
        }}
        className=" hover:bg-black/30 transition-colors text-white p-2 absolute z-10 left-0 top-0 bottom-0"
      >
        <FiChevronLeft />
      </button>
      ...
      {/* <div className="absolute inset-0 z-[5] backdrop-blur-xl">
        <AnimatePresence initial={false} custom={trend}>
          <motion.img
            ...
            style={{ y: "-50%", x: "-50%" }}
            className=" aspect-[4/5] max-h-[90%] max-w-[calc(100%_-_80px)] mx-auto bg-black object-cover shadow-2xl absolute left-1/2 top-1/2"
          />
        </AnimatePresence>
      </div> */}
      <button
        ...
        className=" hover:bg-black/30 transition-colors text-white p-2 absolute z-10 right-0 top-0 bottom-0"
      >
        <FiChevronRight />
      </button>
